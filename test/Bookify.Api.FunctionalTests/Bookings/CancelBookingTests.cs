...
public class CancelBookingTests : BaseFunctionalTest
{
    public CancelBookingTests(FunctionalTestWebAppFactory factory)
        : base(factory)
    {
    }

    [Fact]
    public async Task Cancel_ShouldReturnUnauthorized_WhenAccessTokenIsMissing()
    {
        // Act
    ...
    [Fact]
    public async Task Cancel_ShouldReturnBadRequest_WhenBookingIsNotConfirmed()
    {
        // Arrange
    ...
    [Fact]
    public async Task Cancel_ShouldReturnNoContent_WhenBookingIsConfirmed()
    {
        // Arrange
        DateOnly startDate = GetNextStartDate();
        Guid bookingId = await ReserveBookingAsync(startDate, startDate.AddDays(2));
